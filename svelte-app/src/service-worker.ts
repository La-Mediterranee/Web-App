...
import { ExpirationPlugin as Plugin } from 'workbox-expiration';
...
async function routeAndCacheProductImages({ rotationCount = 200, daysDuration = 30 }) {
	// const imgDbs = [localforage, imgExpirationDb];
	...
	const save = async (url: URL, blob: Promise<Blob>) => {
		...
		// return localforage.length().then((size) => {
		// 	const key = `${url}`;
		// 	const timestamp = Date.now();
		...
		// 			.keys()
		// 			.then((keys) => keys[0])
		// 			.then((outdatedItemKey) => {
		// 				imgDbs.forEach((db) => db.removeItem(outdatedItemKey));
		// 				return outdatedItemKey;
		// 			});
		// 	}

		// 	imgExpirationDb.setItem(key, timestamp);
